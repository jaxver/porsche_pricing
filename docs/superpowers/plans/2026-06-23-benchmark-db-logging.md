# Benchmark DB Logging Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a SQLite-backed benchmark run log with schema management, inserts, and read helpers.

**Architecture:** Keep all database access in one focused module, `benchmark_db.py`, so the logging API is small and easy to test. Use a tiny connection helper and small private row-mapping helpers to keep the public functions straightforward and idempotent. Capture `git_commit` automatically via a private helper that falls back to `NULL` when the repo cannot be queried.

**Tech Stack:** Python stdlib `sqlite3`, `pathlib.Path`, `subprocess`, `typing.Any`, `pandas`.

---

### Task 1: Create the SQLite logging module

**Files:**
- Create: `elferspot_listings/modeling/benchmark_db.py`

- [ ] **Step 1: Implement the public API and private helpers**
...
def get_latest_run(db_path: str | Path) -> dict[str, Any] | None:
    ...
    with _connect(db_path) as conn:
        ...
        run_data = dict(zip(columns, run_row, strict=True))
...
### Task 2: Add SQLite logging tests

**Files:**
- Create: `tests/test_benchmark_db.py`

- [ ] **Step 1: Write the test coverage for schema, inserts, retrieval, and git commit capture**
...
from elferspot_listings.modeling.benchmark_db import (
    ensure_schema,
    get_latest_run,
    get_run_history,
    insert_metrics,
...
- [ ] **Step 1: Run the benchmark DB tests**
...
Run: `git add elferspot_listings/modeling/benchmark_db.py tests/test_benchmark_db.py docs/superpowers/plans/2026-06-23-benchmark-db-logging.md && git commit -m "feat: add benchmark_db module for SQLite run logging"`
